...
public class Weapon : Item
{
    ...
    void OnCollisionEnter2D(Collision2D other)
    {
        if(equipped)
        {
            ...
            Debug.Log(gameObject.tag + " hit: " + other.gameObject.tag + " will apply damage if last tag isnt: " + TagToCheck);
            if (other.gameObject.tag != TagToCheck)
            {
                Debug.Log(gameObject.tag + " hit: " + other.gameObject.tag + " with: " + WeaponDamage + " damage");
